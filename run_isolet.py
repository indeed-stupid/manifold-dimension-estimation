...
Ks_extra = [275, 300, 325, 350, 375, 400, 425, 450, 475, 500]
...
if True:
    ...
    for _type in [0]:
        if _type == 0:
            # Load ISOLET data
            data = np.loadtxt('isolet1+2+3+4.data', delimiter=',')
            # Separate features and labels
            sample = data[:, :-1]               # Feature matrix (num_samples, 616)
            y = data[:, -1].astype(int) 
        ...
        for count in range(10):
            ...
            try:
                hp_13[_type, count] = q_estimator_parallel_v13(ps[_type], n, Ks_extra[count], sample, num_neighborhoods=n)
                print(hp_13[_type, count])
            except Exception as e:
                print(e)
                while True:
                    try:
                        data = sample + np.random.normal(0, 1e-12, size=sample.shape)
                        hp_13[_type, count] = q_estimator_parallel_v13(ps[_type], n, Ks_extra[count], data, num_neighborhoods=n)
                        print("correction success")
                        break
                    except Exception as e:
                        print("correction failure")
            ...
            try:
                hp_14[_type, count] = tls_estimator_parallel_v13(ps[_type], n, Ks_extra[count], sample, num_neighborhoods=n)
                print(hp_14[_type, count])
            except Exception as e:
                print(e)
                while True:
                    try:
                        data = sample + np.random.normal(0, 1e-12, size=sample.shape)
                        hp_14[_type, count] = tls_estimator_parallel_v13(ps[_type], n, Ks_extra[count], data, num_neighborhoods=n)
                        print("correction success")
                        break
                    except Exception as e:
                        print("correction failure")
    ...
    for _type in [0]:
        ...
        for index in [0]:
            ...
            for estimator in [0, 1, 3, 4, 5, 6, 8, 12, 13, 14]:
                ...
                if estimator in [5, 9]:
                    parameters = [0]
                if estimator in [12, 13]:
                    parameters = Ks_extra[int(ranges[estimator, _type * 2]):int(ranges[estimator, _type * 2 + 1])]
                if estimator not in [2, 5, 8, 9, 12, 13]:
    ...
    np.savetxt("isolet_01.csv", ds_01, delimiter=",")
    np.savetxt("isolet_02.csv", ds_02, delimiter=",")
    np.savetxt("isolet_03.csv", ds_03, delimiter=",")
    np.savetxt("isolet_04.csv", ds_04, delimiter=",")
    np.savetxt("isolet_05.csv", ds_05, delimiter=",")
    np.savetxt("isolet_06.csv", ds_06, delimiter=",")
    np.savetxt("isolet_07.csv", ds_07, delimiter=",")
    np.savetxt("isolet_08.csv", ds_08, delimiter=",")
    np.savetxt("isolet_09.csv", ds_09, delimiter=",")
    np.savetxt("isolet_10.csv", ds_10, delimiter=",")
    np.savetxt("isolet_11.csv", ds_11, delimiter=",")
    np.savetxt("isolet_12.csv", ds_12, delimiter=",")
    np.savetxt("isolet_13.csv", ds_13, delimiter=",")
    np.savetxt("isolet_14.csv", ds_14, delimiter=",")
    np.savetxt("isolet_15.csv", ds_15, delimiter=",")
