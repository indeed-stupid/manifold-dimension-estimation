...
# d: individual - intrinsic dimension, sphere, fixed sample size
...
if True:
    ...
    for d in range(2, 21, 2):
        n = 1000 # K = 100 (10 for DanCo), alpha = 5 
        p = 2 * d
        K = 100
        for index in range(0, 100):
            ...
            for estimator in [14, 15, 16]:
                ...
                if estimator == 0:
                    try:
                        lPCA = skdim.id.lPCA(ver='FO').fit_transform_pw(sample, n_neighbors = 100, n_jobs=-1)
                ...
                if estimator == 1:
                    MLE = skdim.id.MLE().fit_transform_pw(sample, n_neighbors = 100, n_jobs=-1)
                ...
                if estimator == 3:
                    MADA = skdim.id.MADA().fit_transform_pw(sample, n_neighbors=100, n_jobs=-1)
                    dim.append(np.mean(MADA))
                
                if estimator == 4:
                    TLE = skdim.id.TLE().fit_transform_pw(sample, n_neighbors=100, n_jobs=-1)
                ...
                if estimator == 6:
                    dim.append(np.mean(ESS(-1, p, n, 100, sample)))
                    
                if estimator == 7:
                    dim.append(np.mean(ABID(-1, p, n, 100, sample)))
    ...
    np.savetxt("d01_01.csv", ds_01, delimiter=",")
    np.savetxt("d01_02.csv", ds_02, delimiter=",")
    np.savetxt("d01_03.csv", ds_03, delimiter=",")
    np.savetxt("d01_04.csv", ds_04, delimiter=",")
    np.savetxt("d01_05.csv", ds_05, delimiter=",")
    np.savetxt("d01_06.csv", ds_06, delimiter=",")
    np.savetxt("d01_07.csv", ds_07, delimiter=",")
    np.savetxt("d01_08.csv", ds_08, delimiter=",")
    np.savetxt("d01_09.csv", ds_09, delimiter=",")
    np.savetxt("d01_10.csv", ds_10, delimiter=",")
    np.savetxt("d01_15.csv", ds_15, delimiter=",")
    np.savetxt("d01_16.csv", ds_16, delimiter=",")
    np.savetxt("d01_17.csv", ds_17, delimiter=",")
