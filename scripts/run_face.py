...
if True:
    ...
    for _type in [0]:
        ...
        for count in range(10):
            ...
            try:
                hp_14[_type, count] = tls_estimator_parallel_v14(ps[_type], n, Ks[count], sample, num_neighborhoods=n)
            except Exception as e:
                print(e)
                while True:
                    try:
                        data = sample + np.random.normal(0, 1e-12, size=sample.shape)
                        hp_14[_type, count] = tls_estimator_parallel_v14(ps[_type], n, Ks[count], data, num_neighborhoods=n)
                        print("correction success")
                        break
                    except Exception as e:
                        print("correction failure")
